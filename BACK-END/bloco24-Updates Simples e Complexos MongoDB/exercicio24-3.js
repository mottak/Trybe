...
db.movies.find({ category: { $all: ["action"]}, imdbRating: { $gte: 7 } });
...
db.movies.updateOne({ title: "Batman" }, { $push: {ratings: [85, 100, 102, 105] }})
...
db.movies.find({ ratings: { $gte: 100, $lte: 105 } }, { _id: 0, title: 1, ratings:1 })

// 8-Retorne todos os filmes com ratings entre 64 e 105 e divisíveis por 9 , exibindo apenas os campos title e ratings .
db.movies.find({ $and: [{ratings: { $gte: 64, $lte: 105 }}, {ratings: { $mod: [9, 0]}}] },
...
db.movies.find({ $and: [{category: "adventure"}, {ratings: { $gte: 103 }}]  },
...
db.movies.find({ $or: [{category: "sci-fi"}, {ratings: { $gte: 199 }}]  },
  { _id: 0, title: 1, ratings: 1, category: 1 });
